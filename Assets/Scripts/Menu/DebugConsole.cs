...
public class DebugConsole : MonoBehaviour
{
    ...
    void OnGUI()
    {
        ...
        if ( _isOpen )
        {
            _windowRect = GUI.Window( -1111, _windowRect, MainWindow, "Debug Console");
    ...
    private void MainWindow( int windowID )
    {
        if ( _mode == Mode.Log )
        {
            ...
            if ( GUILayout.Button( "Enter", GUILayout.Width( 70.0F ), GUILayout.Height( 20.0F ) ) || Input.GetKeyUp(KeyCode.Return))
            {
            ...
            if ( GUILayout.Button( "Watch Vars", GUILayout.Width( 120.0F ), GUILayout.Height( 20.0F ) ) )
            {
                _mode = Mode.WatchVars;
            }
			if( GUILayout.Button( "Close", GUILayout.Width( 95.0f), GUILayout.Height(20.0f)))
			{
				CMDClose();
			}
        ...
        else if ( _mode == Mode.CopyLog )
        {
            ...
            GUILayout.TextArea( _displayString.ToString(), GUILayout.ExpandWidth( false ), GUILayout.ExpandHeight( true ), GUILayout.Width( 520.0F ) );
            GUILayout.EndScrollView();
            ...
            if ( GUILayout.Button( "Back", GUILayout.Width( 95.0F ), GUILayout.Height( 20.0F ) ) )
            {
                _mode = Mode.Log;
            }
    ...
    public void LogMessage( string text, MessageTypes messageType, Color displayColor, bool useCustomColor )
    {
        if ( _messages != null )
        {
            if ( useCustomColor )
            {
                _messages.Add( new Message(Time.time + ":> " + text, messageType, displayColor ) );
            }
            else
            {
                _messages.Add( new Message(Time.time + ":> " + text, messageType ) );
            }
